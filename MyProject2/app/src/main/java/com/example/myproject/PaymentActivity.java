...
public class PaymentActivity extends Activity
{
    ...
    private void errorCheck()
    {
        ...
        {
            case R.id.rad_debit: cardType = "debit";break;
            case R.id.rad_visa: cardType = "visa";break;
            case R.id.rad_mastercard: cardType = "masterCard";break;
        }
        // set error when cardNumber is null or doesn't equal 16 characters
        if (cardNumber.length() != 16 || cardNumber == null) { edtCardNumber.setError("Card number has to be 16 digits"); }
        // set error when expiryDate is null
        if (expiryDate.length() == 0 || expiryDate == null) { txtDisplayDate.setError("Expiry date can not be empty"); }
        // set error when cvcCode is null or does't equal 3 characters
        if (cvcCode.length() != 3 || cvcCode == null) { edtCvcCode.setError("CVC code has to be 3 digits"); }
        // if it passes these requirements, update and launch ConfirmationActivity
        else if (cardNumber.length() == 16 && expiryDate.length() != 0 && cvcCode.length() == 3) { launchConfirmationActivity(); }
    }
}
